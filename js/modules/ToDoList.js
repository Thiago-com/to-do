...
export default class TodoList {
  ...
  async deleteTask({ target }) {
    try {
      await fetch(`http://localhost:3333/todos/${target.
      parentElement.id}`, {
        method: "DELETE",
      });
      this.getTasks();
...
try {
  ...
  addUpdateTaskEvents() {
    ...
    checkboxes.forEach((checkbox) =>
      description.addEventListener("click", (event) => this.updateTask(event)),
    );
  }
